...
export async function saveLogsToSpreadsheet(spreadsheetId: string, token: string, logs: AttendanceRecord[]) {
  try {
    ...
    for (const [monthStr, monthLogs] of Object.entries(logsByMonth)) {
      // Find existing sheet matching normalized name
      let targetSheetName = existingSheets.find(s => normSheetName(s) === normSheetName(monthStr));
      ...
      if (!targetSheetName) {
        try {
          ...
          targetSheetName = monthStr;
          existingSheets.push(monthStr); // Mark as created

          // Write headers
          const headersUrl = `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${encodeURIComponent(targetSheetName)}!A1:E1?valueInputOption=USER_ENTERED`;
          ...
          // Fallback: re-fetch sheet titles if race condition occurred
          try {
            const reMeta = await sheetsFetch(metaUrl, token);
            const reSheets: string[] = reMeta.sheets?.map((s: any) => s.properties?.title || "") || [];
            targetSheetName = reSheets.find(s => normSheetName(s) === normSheetName(monthStr)) || monthStr;
            if (!existingSheets.includes(targetSheetName)) existingSheets.push(targetSheetName);
          } catch {
            targetSheetName = monthStr;
          }
        }
      ...
      try {
        const clearUrl = `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${encodeURIComponent(targetSheetName)}!A2:E2000:clear`;
        await sheetsFetch(clearUrl, token, { method: "POST" });
      } catch (err) {
        console.warn(`Could not clear logs range in ${targetSheetName}:`, err);
      }

      // 4. Write new values
      if (values.length > 0) {
        const writeUrl = `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${encodeURIComponent(targetSheetName)}!A2:E2000?valueInputOption=USER_ENTERED`;
